...
One **backbone** (the `Context/*.yaml` files), many **lenses**. Each job application is
**one self-contained folder under `Applications/`** — source, output, and outcome together.
Claude only ever *proposes* an `overlay.yaml`; you approve; `main.py` deterministically
merges and renders. The backbone and the live site at https://jwhwang91.github.io are never
touched. (Reusable flat lenses still live here in `Context/variants/`; see below.)

## Layout

```
Applications/<name>/            <- one folder per application (e.g. "Tesla 1") — gitignored
    ...
    result.md       # the outcome — YOU update this (status / dates / stage / why / next move)
    resume.html     # generated: lean, self-contained, PDF-ready; case-study links -> live portfolio
    (any PDF / evidence files you add)
```

A flat `Context/variants/<name>.yaml` (no folder) also works for a reusable field lens you
might want to commit publicly (e.g. `adas-controls.yaml`); it renders to
`Applications/<name>/resume.html` like any other.

## The loop

1. **Ingest** — `python main.py --new-variant "Tesla 1"`, then paste the JD into
   `Applications/Tesla 1/jd.txt`.
...
5. **Feedback** — your edits/notes go in `notes.md`, and the outcome in `result.md`, so the
   next application starts smarter.

`python main.py --list-variants` shows every application and whether it's been built.
